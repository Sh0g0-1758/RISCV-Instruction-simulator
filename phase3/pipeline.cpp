...
void SecondStage(IDEX &idex, int program_counter, IFID ifid)
{
    ...
    idex.change_CW(ifid.IR.substr(25));
...
int main()
{
    ...
    for (int program_counter = 0; program_counter < instructionVector.size(); program_counter++)
    {
        ...
        {
            if (i == 0) threads[i] = std::thread(FirstStage, std::ref(ifid), (instructionVector[program_counter]), (program_counter));
            if (i == 1) threads[i] = std::thread(SecondStage, std::ref(idex), (program_counter), (ifid));
            if (i == 2) threads[i] = std::thread(ThirdStage, std::ref(exmo), (idex), std::ref(program_counter), instructionVector.size());
            if (i == 3) threads[i] = std::thread(FourthStage, std::ref(mowb), (exmo));
            if (i == 4) threads[i] = std::thread(FifthStage, (mowb));
        }
